...
;(function(w){
    w.$summer = w.$summer || {};
    // w.$summer.initEnv = function(){
    //     w.cordova = {
    //         require:function(x){
    //             console.log(x + " require done! ");
    //             return {};
    //         }
    //     }
    // }
    ...
    s.openComponent = function(params){
        if($summer.os =="ios" || $summer.os =="android"){
            ...
            if(!appComponentManager){
                alert("pc err, appComponentManager is not exist !")
                return;
            }
            appComponentManager.openComponent(params);
        }else{
            alert("openComponent not support!")
        }
    }
    s.closeComponent = function(params){
        if($summer.os =="ios" || $summer.os =="android"){
            summer.callService("YUC.close", params);
            return;
        }else if($summer.os == "pc"){
            if(!appComponentManager){
                alert("pc err, appComponentManager is not exist !")
            ...
            appComponentManager.closeComponent(params);
        }else{
            alert("closeComponent not support!")
        }
    }
}(window,summer);
	...
	s.UMService = {
		...
		callAction : function(controllerName, actionName, params, isDataCollect, callbackActionID, contextmapping, customArgs){
			if(arguments.length == 1 && typeof arguments[0] == "object"){
				...
				//$service.call("UMService.callAction","{callback:'myback', contextmapping:'data'，viewid:'"+controllerName+"',isDataCollect:'false',params:{demo1:'demo1'},action:'needPwd'}");
